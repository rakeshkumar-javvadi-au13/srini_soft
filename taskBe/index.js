...
app.post('/invoices', async (req, res) => {
    ...
    try {
        if (!req.body.receviable || !req.body.payable) {
...
app.delete('/invoices/receviable/:id', async (req, res) => {
    try {
        if (!req.params.id) {
            return res.status(400).send("Deleted id required");
        }
        const invoice = await Invoices.findByIdAndUpdate(req.params.id, { $unset: { receviable: 1 } }, { new: true });
        res.status(200).send(invoice);
    } catch (error) {
        res.status(500).send(error);
    }
